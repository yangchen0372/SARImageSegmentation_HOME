...
def process_label(rgb_mask, classes,classes_idx,colormap):
    '''
    将彩色mask映射到对应的类别索引
    ...
    # # 该循环为了演示每一类标签图
    # for class_index, cmap in enumerate(output_mask):
    #     plt.title('{}'.format(fusar_config.CLASSES[class_index]))
...
class FUSAR_DATASET(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        plt.show()
        ## debug end
        mask = process_label(mask,self.classes,self.classes_idx,self.colormap)
